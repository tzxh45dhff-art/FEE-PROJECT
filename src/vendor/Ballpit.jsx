/*
 * Ballpit — vendored from React Bits (reactbits.dev), MIT.
 * Component inspired by Kevin Levron: https://x.com/soju22/status/1858925191671271801
 *
 * Kept verbatim apart from this header. It is third-party code and is excluded
 * from the TypeScript project; its contract is declared in src/types/vendor.d.ts.
 */
...
class x {
  ...
  dispose() {
    ...
    /*
     * DEVIATION FROM UPSTREAM — deliberately not calling forceContextLoss().
     *
     * It destroys the canvas's WebGL context permanently: a canvas that has
     * been through it can never be given another one. React's development
     * StrictMode runs setup, teardown, then setup again on the *same* DOM
     * node, so the second setup built a renderer on a dead canvas and three
     * threw reading `precision` off a null capabilities object — taking the
     * whole landing page down to the error boundary.
     *
     * `dispose()` on the line above already releases the GPU resources. The
